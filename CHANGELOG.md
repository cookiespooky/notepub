...
- Fixed modern template infrastructure generated by `template update`:
  ...
  - removed incorrect root-relative interpretation that could skip media export from content directory.
- Fixed generated modern deploy workflow content sync path resolution:
  - workflow now resolves absolute content path from config before content-repo sync.

### Changed

- Added regression assertions for modern template update output to prevent path-resolution regressions in generated `build.sh` and `deploy.yml`.
