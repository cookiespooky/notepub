...
export async function POST(req: NextRequest) {
  ...
  // Publish respects checkbox: caller sets draft flag explicitly.
  if (typeof nextFrontmatter.draft === "undefined") {
    nextFrontmatter.draft = false;
  }

  const notes = await listRawNotesWithDrafts(context.prefix || undefined);
  const resolvedPath = normalizePath(targetPath || path) || ensureRootFilename(nextFrontmatter.title || "");
  const normalizedSlug = nextFrontmatter.slug.toString().trim();
  if (slugExists(notes, normalizedSlug, path || resolvedPath)) {
