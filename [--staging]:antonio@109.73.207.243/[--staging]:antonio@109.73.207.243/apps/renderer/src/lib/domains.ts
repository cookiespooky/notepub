...
export function getSitesBaseDomain(): string {
  return process.env.SITES_BASE_DOMAIN || process.env.NEXT_PUBLIC_SITES_BASE_DOMAIN || DEFAULT_SITES_BASE_DOMAIN;
}
...
export function buildSiteUrl(slug: string, protocol = "https"): string {
  return `${protocol}://${buildSiteHost(slug)}`;
}
