...
export async function GET(_req: NextRequest) {
  ...
  for (const entry of index.flat) {
    const isRootIndex = entry.relativeKey.toLowerCase() === "index.md";
    const loc = isRootIndex ? "/" : `/${entry.slug}`;
    urls.add(new URL(loc, origin).toString());
  }

  // include folder listing pages
  const folderMeta = index.folderMeta;
  for (const [pathKey] of folderMeta) {
    const slugPath = pathKey
      .split("/")
      .filter(Boolean)
      .map((segment) => segment);
    if (slugPath.length === 0) continue;
    const loc = `/folders/${slugPath.join("/")}`;
    urls.add(new URL(loc, origin).toString());
  }
