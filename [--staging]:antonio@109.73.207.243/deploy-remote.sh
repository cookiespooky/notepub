...
if pm2 describe "$(pm2_app_name notepub-dashboard)" >/dev/null 2>&1; then
  ...
  pm2 start npm --name "$(pm2_app_name notepub-dashboard)" -- run start --workspace @notepub/dashboard -- --hostname 127.0.0.1 --port 3100
fi

if pm2 describe "$(pm2_app_name notepub-renderer)" >/dev/null 2>&1; then
  pm2 reload "$(pm2_app_name notepub-renderer)" --update-env
else
  pm2 start npm --name "$(pm2_app_name notepub-renderer)" -- run start --workspace @notepub/renderer -- --hostname 127.0.0.1 --port 3200
fi
